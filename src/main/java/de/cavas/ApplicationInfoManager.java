...
//            this.instanceInfo = new EurekaConfigBasedInstanceInfoProvider(config).get();
     ...
     */
    public synchronized void setInstanceStatus(de.cavas.InstanceInfo.InstanceStatus status) {
        de.cavas.InstanceInfo.InstanceStatus prev = instanceInfo.setStatus(status); // setStatus(status);
        if (prev != null) {
