...
//            this.instanceInfo = new EurekaConfigBasedInstanceInfoProvider(config).get();
     ...
     */
    public synchronized void setInstanceStatus(eurekademo.InstanceInfo.InstanceStatus status) {
        eurekademo.InstanceInfo.InstanceStatus prev = instanceInfo.setStatus(status); // setStatus(status);
        if (prev != null) {
